                                ...
                                Index m, Number* g_l, Number* g_u)
{
  ...
  for(size_t ii = 0;ii<ele->x.size();ii++){
    ...
    x_l[3*ii] = -3 - ele->x[ii][0];
    x_l[3*ii+1] = -3- ele->x[ii][1];//-10;
    x_l[3*ii+2] = -3 - ele->x[ii][2];


    //all variables upper bound 10 (arbitrary)
    x_u[3*ii] = 3 - ele->x[ii][0];
    x_u[3*ii+1] = 3 - ele->x[ii][1];
    x_u[3*ii+2] = 3 - ele->x[ii][2];
          ...
          IpoptCalculatedQuantities* ip_cq)
{
  ...
  for (size_t ii=0; ii<ele->x.size(); ii++) {
    for(int jj = 0;jj<3;jj++){
      ele->v[ii][jj] = (1.0/ele->dt) * (x[3*ii+jj]);
      ele->x[ii][jj] = x[3*ii+jj] + ele->x[ii][jj];
    }
  }
